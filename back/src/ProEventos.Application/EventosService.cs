...
namespace ProEventos.Application
{
    ...
    {
        private readonly IGeralPersist _geralPersist; private readonly IEventoPersist _eventoPersist;
        public EventosService(IGeralPersist geralPersist, IEventoPersist eventoPersist){
            _geralPersist = geralPersist; _eventoPersist = eventoPersist; 
        } 
        
        public async Task<Evento> AddEventos(Evento model){
            try {
                _geralPersist.Add<Evento>(model);
                return (await _geralPersist.SaveChangesAsync()) ? await _eventoPersist.GetEventoByIdAsync(model.Id, false) : null;
            }
            catch (Exception ex){
